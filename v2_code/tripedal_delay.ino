...
void loop() {
  if (Serial.available() > 0) {
    ...
    while (data.length() > 0 && servoIndex < 9) {
      ...
      if (commaIndex == -1) {
        positions[servoIndex++] = constrain(data.toInt(), 0, 180);  // Constrain to safe range
        break;
      } else {
        positions[servoIndex++] = constrain(data.substring(0, commaIndex).toInt(), 0, 180);
    ...
    for (int i = 0; i < 9; i++) {
      ...
      delay(15); // Short delay for servo to reach its position
    }
  }
}
